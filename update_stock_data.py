...
def fetch_one_fundamentals(yahoo_ticker):
    """개별 종목의 PER/PEG/포워드PER/EPS/다음 실적발표일/섹터/배당(배당수익률/다음 배당락일 예상/직전 배당금/전 배당대비 등락률)/
    ...
    # 직전 배당금, 그 이전 배당 대비 등락률, 다음 배당락일 예상
    # (야후 API는 미래 배당락일을 제공하지 않고 가장 최근 지난 배당락일만 제공하므로,
    #  과거 배당 이력의 지급 주기(월배당/분기배당 등)를 바탕으로 다음 배당락일을 추정한다)
    last_dividend_value = info.get("lastDividendValue")
    dividend_change_pct = None
    next_ex_dividend_estimated = None
    if last_dividend_value:
        try:
            ...
            recent_dates = divs.index[-6:]  # 최근 최대 6회 지급일로 주기 추정 (표본 적으면 그만큼만 사용)
            if len(recent_dates) >= 2:
                ...
                last_date = recent_dates[-1].date()
                next_ex_dividend_estimated = (last_date + timedelta(days=round(median_interval))).isoformat()
        except Exception:
            pass
        # 야후가 이미 공식 발표한 미래 배당락일을 제공하는 경우(예: 애플처럼 분기 배당 일정이
        # 미리 확정된 종목) 추정치보다 그 공식 값이 더 정확하므로 우선 사용
        if ex_dividend_date and ex_dividend_date >= date.today().isoformat():
            next_ex_dividend_estimated = ex_dividend_date
    ...
    return {
        ...
        "nextExDividendDateEstimated": next_ex_dividend_estimated,
